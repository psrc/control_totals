...
def load_targets(pipeline):
    ...
    """
    p = pipeline
    emp_res_con = load_base_year_emp(p,'res_con')
    emp_no_res_con = load_base_year_emp(p,'no_res_con')
    emp = pd.concat([emp_res_con, emp_no_res_con])
    res_con_df = p.get_table('adjusted_emp_change_targets_res_con')
    no_res_con_df = p.get_table('adjusted_emp_change_targets_no_res_con')
    df = pd.concat([res_con_df,no_res_con_df])
    df = df.merge(emp, on='target_id')
    return df
